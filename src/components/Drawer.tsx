...
export function Drawer({ open, onClose, title, subtitle, children, footer }: DrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      />
      <aside
        className="absolute right-0 top-0 flex h-full w-full max-w-xl flex-col bg-ivory shadow-lift animate-drawer-in"
        role="dialog"
        aria-modal="true"
        aria-label={title}
      >
        <header className="flex items-start justify-between gap-4 border-b border-soft-border bg-card px-6 py-5">
          <div>
        ...
        {footer && <footer className="border-t border-soft-border bg-card px-6 py-4">{footer}</footer>}
      </aside>
    </div>
  )
}
