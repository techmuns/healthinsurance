...
const NAMES: Record<string, string> = {
  ...
  'aditya-birla': 'Aditya Birla', 'manipalcigna': 'ManipalCigna', panel: 'Panel',
}
const pretty = (id: string) => NAMES[id] ?? id
const GOLD = '#C99736' // reserved for "look here — the best read", never a level

// Conviction tone (colour psychology): teal = confident, navy = steady, slate =
// tentative. Gold is kept for attention only, so it never competes with a level.
const CONVICTION: Record<Insight['conviction'], { bg: string; fg: string; dot: string; label: string }> = {
  ...
  low: { bg: 'rgba(140,151,168,0.16)', fg: '#5B6573', dot: '#8C97A8', label: 'Low conviction' },
}
...
function InsightCard({ ins, hero = false }: { ins: Insight; hero?: boolean }) {
  ...
  // A single-subject insight gets a gold spotlight in its chart; comparison reads stay multi-tone.
    ...
    <article
      className={[
        ...
        hero
          ...
          : 'border-soft-border shadow-card hover:shadow-[0_10px_30px_rgba(23,43,77,0.10)]',
      ].join(' ')}
    >
      {/* Gold spine on the top read — the eye lands here first. */}
      {hero && <span className="absolute inset-x-0 top-0 h-[3px] bg-gradient-to-r from-[#E7C879] via-[#C99736] to-[#E7C879]" />}

      <div className="border-b border-soft-border px-4 py-3">
        <div className="mb-1.5 flex flex-wrap items-center gap-2">
          <span className={['inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9.5px] font-bold uppercase tracking-[0.1em]', hero ? 'bg-champagne-soft text-champagne-deep' : 'bg-navy-primary/8 text-navy-primary'].join(' ')}>
            {hero && <span className="h-1.5 w-1.5 rounded-full" style={{ background: GOLD }} />}#{ins.rank} · {CATEGORY_LABEL[ins.category]}
          </span>
          ...
          {hero && <span className="ml-auto inline-flex items-center gap-1 text-[9.5px] font-bold uppercase tracking-[0.1em] text-champagne-deep"><Sparkles className="h-3 w-3" />Top read</span>}
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 px-4 py-3 lg:grid-cols-[1.15fr_1fr]">
        <div className="space-y-3">
          ...
          <div className="rounded-lg bg-soft-blue/40 px-3 py-2">
            <p className="flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-[0.1em] text-navy-primary"><Sparkles className="h-3 w-3" /> What the market misses</p>
            <p className="mt-1 text-[12px] leading-snug text-navy-deep">{ins.whatConsensusMisses}</p>
          </div>
          <div>
            <p className="mb-1 text-[9px] font-bold uppercase tracking-[0.1em] text-ink-secondary">The numbers</p>
            <ul className="space-y-1.5">
              {ins.evidence.map((e, i) => (
                <li key={i} className="flex flex-wrap items-center gap-x-2 gap-y-1 rounded-lg border border-soft-border bg-ice/40 px-2.5 py-1.5 text-[11px]">
                  <span className="font-semibold text-navy-deep">{pretty(e.insurer)}</span>
        ...
        <div className="flex flex-col gap-2">
          <InsightChart spec={ins.chart} focal={focal} />
        </div>
      </div>

      <div className="space-y-1.5 border-t border-soft-border bg-ice/30 px-4 py-2.5">
        <p className="flex items-start gap-1.5 text-[11px] leading-snug text-ink-primary"><Crosshair className="mt-0.5 h-3.5 w-3.5 shrink-0 text-coral" /><span><strong className="text-navy-deep">What would prove it wrong:</strong> {ins.falsifier}</span></p>
...
export function Insights() {
  ...
  const bestCoveredId = FILE.meta.coverage.reduce((a, b) => (b.readyPct > a.readyPct ? b : a), FILE.meta.coverage[0])?.insurer
      ...
      <header className="card-surface rounded-2xl p-4 sm:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="max-w-2xl text-[12.5px] text-ink-secondary">What the market is missing across India's five standalone health insurers. Every read is checked against the numbers — and names the one thing that would prove it wrong. Built from {FILE.meta.signalsComputed} signals; the AI writes only what the data supports.</p>
          </div>
          <div className="rounded-xl border border-soft-border bg-white/70 px-3 py-2 text-right">
            ...
            <p className="mt-1 text-[10px] text-ink-secondary">{avgReady}% source-backed</p>
          </div>
        </div>
        {/* coverage bar */}
        <div className="mt-3 flex flex-wrap gap-2">
          {FILE.meta.coverage.map((c) => {
            const best = c.insurer === bestCoveredId
            return (
              <span key={c.insurer} className={['inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10.5px] text-ink-secondary ring-1', best ? 'bg-champagne-soft ring-[#E7D29B]' : 'bg-ice ring-soft-border'].join(' ')}>
                <span className="h-1.5 w-1.5 rounded-full" style={{ background: best ? GOLD : c.readyPct >= 90 ? '#168E8E' : c.readyPct >= 75 ? '#27457E' : '#A8443B' }} />
                {pretty(c.insurer)} <strong className="text-navy-deep">{c.readyPct}%</strong>{c.gapped ? ` · ${c.gapped} gap` : ''}
              </span>
            )
          })}
        </div>
